...
CallbackListener cb = new CallbackListener() { //used by ControlP5 to clear text field on double-click
    public void controlEvent(CallbackEvent theEvent) {
        ...
        } else if (cp5.isMouseOver(cp5.get(Textfield.class, "fileNameBitalino"))){
            println("CallbackListener: controlEvent: clearing bitalino");
            cp5.get(Textfield.class, "fileNameBitalino").clear();
...
Button protocolBluetoothGanglion;
Button protocolBLED112Bitalino;
...
Button latencyGanglion5ms; //fekr konam baraye bitalino nemikhad
...
public void controlEvent(ControlEvent theEvent) {
    ...
    if (theEvent.isFrom("")) {
        ...
        } else if(newDataSource == DATASOURCE_BITALINO){
            updateToNChan(1);
        }
    ...
    if (theEvent.isFrom("bleList")) {
        Map bob = ((MenuList)theEvent.getController()).getItem(int(theEvent.getValue()));
        ble_portName = (String)bob.get("headline");
        output("BLE Device Name = " + ble_portName);
    }
...
class ControlPanel {
    ...
    ControlPanel(OpenBCI_GUI mainClass) {
        ...
        interfaceBoxBitalino = new InterfaceBoxBitalino(x + w, dataSourceBox.y, w, h, globalPadding);
        ...
        // Bitalino
        bleBox = new BLEBox(x + w, interfaceBoxBitalino.y + interfaceBoxBitalino.h, w, h, globalPadding);
        dataLogBoxBitalino = new SessionDataBox(x + w, (bleBox.y + bleBox.h), w, h, globalPadding, DATASOURCE_BITALINO);
        sampleRateBitalinoBox = new SampleRateBitalinoBox(x + w, (dataLogBoxBitalino.y + dataLogBoxBitalino.h), w, h, globalPadding);
        // latencyBitalinoBox = new LatencyBitalinoBox(x + w, (sampleRateBitalinoBox.y + sampleRateBitalinoBox.h), w, h, globalPadding);
        // wifiTransferProtcolBitalinoBox = new WifiTransferProtcolBitalinoBox(x + w, (latencyBitalinoBox.y + latencyBitalinoBox.h), w, h, globalPadding);
    }
    ...
    public void draw() {
        ...
        if (systemMode != 10) { // only draw control panel boxes if system running is false
            ...
            if (eegDataSource == DATASOURCE_CYTON) {	//when data source is from OpenBCI
                if(!hub.isHubRunning()) {
                    ...
                    cp5.get(Textfield.class, "fileNameBitalino").setVisible(false);
                    //cp5.get(MenuList.class, "sdTimes").setVisible(true); //make sure the SD time record options menulist is visible
            ...
            } else if (eegDataSource == DATASOURCE_BITALINO) {
                if(!hub.isHubRunning()) {
                    noHubBox.draw();
                }
                else if (bitalino.getInterface() == INTERFACE_NONE) {
                    interfaceBoxBitalino.draw();
                } else {
                    interfaceBoxBitalino.draw();
                    if (bitalino.getInterface() == INTERFACE_HUB_BLED112) {
                        bleBox.y = interfaceBoxBitalino.y + interfaceBoxBitalino.h;
                        dataLogBoxBitalino.y = bleBox.y + bleBox.h;
                        bleBox.draw();
                        cp5.get(MenuList.class, "bleList").setVisible(true);
                        cp5.get(Textfield.class, "staticIPAddress").setVisible(false);
                    }
                    // else if (bitalino.getInterface() == INTERFACE_HUB_WIFI) {
                    //     wifiBox.y = interfaceBoxGanglion.y + interfaceBoxGanglion.h;
                    //     dataLogBoxGanglion.y = wifiBox.y + wifiBox.h;
                    //     wifiBox.draw();
                    //     if (hub.getWiFiStyle() == WIFI_STATIC) {
                    ...
                    //     wifiTransferProtcolGanglionBox.draw();
                    // }
                    dataLogBoxBitalino.draw(); //Drawing here allows max file size dropdown to be drawn on top
                    cp5.get(Textfield.class, "fileNameCyton").setVisible(false); //make sure the data file field is visible
                    cp5.get(Textfield.class, "fileNameGanglion").setVisible(false); //make sure the data file field is visible
                    cp5.get(Textfield.class, "fileNameBitalino").setVisible(true);
                }
            } else {
                //set other CP5 controllers invisible
                hideAllBoxes();
        ...
        if (systemMode != 10 && outputDataSource == OUTPUT_SOURCE_ODF) {
            ...
            } else if (eegDataSource == DATASOURCE_BITALINO && bitalino.getInterface() != INTERFACE_NONE) {
                dataLogBoxBitalino.cp5_dataLog_dropdown.draw();
            }
        }
        ...
        if (systemMode != 10) {
            ...
            //active buttons during DATASOURCE_CYTON
                ...
                if (!hub.isHubRunning()) {
                    ...
                    if (protocolBluetoothGanglion.isMouseHere()) {
                        protocolBluetoothGanglion.setIsActive(true);
                        protocolBluetoothGanglion.wasPressed = true;
                        if(isMac()) {
                            protocolBLEGanglion.setColorNotPressed(colorNotPressed);
                        }
                        protocolBluetoothGanglion.setColorNotPressed(isSelected_color);
                        protocolWifiGanglion.setColorNotPressed(colorNotPressed);
                        protocolBLED112Ganglion.setColorNotPressed(colorNotPressed);
                    }
            ...
            if (eegDataSource == DATASOURCE_SYNTHETIC) {
                ...
                if (!hub.isHubRunning()) {
                    ...
                    if (protocolBLED112Bitalino.isMouseHere()) {
                        protocolBLED112Bitalino.setIsActive(true);
                        protocolBLED112Bitalino.wasPressed = true;
                        // if(isMac()) {
                        //     protocolBLEGanglion.setColorNotPressed(colorNotPressed);
                        // }
                        protocolBLED112Bitalino.setColorNotPressed(isSelected_color);
                        // protocolWifiBitalino.setColorNotPressed(colorNotPressed);
                        // protocolBluetoothGanglion.setColorNotPressed(colorNotPressed);
                    }
                    ...
                    if (sampleRate100.isMouseHere()) {
                        sampleRate100.setIsActive(true);
                        sampleRate100.wasPressed = true;
                        sampleRate100.setColorNotPressed(isSelected_color);
                        // sampleRate1600.setColorNotPressed(colorNotPressed); //default color of button
                    }
    ...
    public void CPmouseReleased() {
        ...
        if (((eegDataSource == DATASOURCE_CYTON || eegDataSource == DATASOURCE_GANGLION) || eegDataSource == DATASOURCE_BITALINO)) {
        ...
        if (protocolBLED112Bitalino.isMouseHere() && protocolBLED112Bitalino.wasPressed) {

            wifiList.items.clear();
            bleList.items.clear();
            controlPanel.hideAllBoxes();
            if (isHubObjectInitialized) {
                output("Protocol BLED112 Selected for Bitalino");
                if (hub.isPortOpen()) hub.closePort();
                bitalino.setInterface(INTERFACE_HUB_BLED112);
                // hub.searchDeviceStart();
            } else {
                outputWarn("Please wait till hub is fully initalized");
            }
        ...
        if (protocolBluetoothGanglion.isMouseHere() && protocolBluetoothGanglion.wasPressed) {

            wifiList.items.clear();
            bleList.items.clear();
            controlPanel.hideAllBoxes();
            if (isHubObjectInitialized) {
                output("Protocol Bluetooth Selected for Ganglion");
                if (hub.isPortOpen()) hub.closePort();
                ganglion.setInterface(INTERFACE_HUB_BLUETOOTH);
                // hub.searchDeviceStart();
            } else {
                outputWarn("Please wait till hub is fully initalized");
            }
        ...
        if (autoSessionName.isMouseHere() && autoSessionName.wasPressed) {
            ...
            if (eegDataSource == DATASOURCE_CYTON) {
                _textField = "fileNameCyton";
            } else if (eegDataSource == DATASOURCE_CYTON) {
                _textField = "fileNameGanglion";
            } else {
                _textField = "fileNameBitalino";
            }
        ...
        if (outputODF.isMouseHere() && outputODF.wasPressed) {
            ...
            if (eegDataSource == DATASOURCE_CYTON) {
                ...
                controlPanel.dataLogBoxBitalino.setToODFHeight();
            }
        }
        ...
        if (outputBDF.isMouseHere() && outputBDF.wasPressed) {
            ...
            if (eegDataSource == DATASOURCE_CYTON) {
                ...
                controlPanel.dataLogBoxBitalino.setToBDFHeight();
            }
        }
        ...
        if (sampleRate100.isMouseHere() && sampleRate100.wasPressed) {
            bitalino.setSampleRate(100);
        }
        ...
        protocolBLED112Bitalino.setIsActive(false);
        protocolBLED112Bitalino.wasPressed = false;
        protocolBLED112Ganglion.setIsActive(false);
        protocolBLED112Ganglion.wasPressed = false;
        protocolBluetoothGanglion.setIsActive(false);
        protocolBluetoothGanglion.wasPressed = false;
        ...
        sampleRate100.setIsActive(false);
        sampleRate100.wasPressed = false;
...
public void initButtonPressed(){
    if (initSystemButton.but_txt == "START SESSION") {
        if (((eegDataSource == DATASOURCE_CYTON && cyton.getInterface() == INTERFACE_NONE) || (eegDataSource == DATASOURCE_GANGLION && ganglion.getInterface() == INTERFACE_NONE) || (eegDataSource == DATASOURCE_BITALINO && bitalino.getInterface() == INTERFACE_NONE))) {
            ...
            initSystemButton.setIsActive(false);
            return;
        } else if (eegDataSource == DATASOURCE_GANGLION && (ganglion.getInterface() == INTERFACE_HUB_BLE || ganglion.getInterface() == INTERFACE_HUB_BLED112) && ble_portName == "N/A") {
            ...
            initSystemButton.setIsActive(false);
            return;
        } else if (eegDataSource == DATASOURCE_BITALINO && bitalino.getInterface() == INTERFACE_HUB_BLED112 && ble_portName == "N/A") {
            output("No BLE device selected. Please select your Bitalino device and retry system initiation.");
            initSystemButton.wasPressed = false;
            initSystemButton.setIsActive(false);
            return;
            ...
            if (eegDataSource == DATASOURCE_CYTON) {
                ...
                verbosePrint("ControlPanel — port is open: " + bitalino.isPortOpen());
                if (bitalino.isPortOpen()) {
                    bitalino.closePort();
                }
                sessionName = cp5.get(Textfield.class, "fileNameBitalino").getText(); // store the current text field value of "File Name" to be passed along to dataFiles
            }
        ...
        cp5.get(Textfield.class, "fileNameBitalino").setText(getDateString());
...
void updateToNChan(int _nchan) {
    ...
    bitalino.initDataPackets(_nchan, 3);
...
class DataSourceBox {
    ...
    DataSourceBox(int _x, int _y, int _w, int _h, int _padding) {
        ...
        sourceList.addItem(makeItem("SYNTHETIC (algorithmic)"));
         sourceList.addItem(makeItem("LIVE (from Bitalino)"));
...
class BLEBox {
    ...
    public void draw() {
        ...
        refreshBLE.draw();
        // println(isHubInitialized, isHubObjectInitialized, hub.isSearching());
        if(isHubInitialized && isHubObjectInitialized && (ganglion.isBLE() || bitalino.isBLE()) && hub.isSearching()){
...
class InterfaceBoxGanglion {
    ...
    InterfaceBoxGanglion(int _x, int _y, int _w, int _h, int _padding) {
        ...
        paddingCount ++;
        protocolBluetoothGanglion = new Button (x + padding, y + padding * paddingCount + buttonHeight * paddingCount, w - padding * 2, 24, "Bluetooth", fontInfo.buttonLabel_size);
        paddingCount ++;
        h += padding + buttonHeight;
    ...
    public void draw() {
        ...
        if (isWindows()){
            protocolBluetoothGanglion.draw();
        }
    }
};

class InterfaceBoxBitalino {
    int x, y, w, h, padding; //size and position

    InterfaceBoxBitalino(int _x, int _y, int _w, int _h, int _padding) {
        x = _x;
        y = _y;
        w = _w;
        padding = _padding;
        ...
        protocolBLED112Bitalino = new Button (x + padding, y + padding * paddingCount + buttonHeight * paddingCount, w - padding * 2, 24, "BLED112 Dongle", fontInfo.buttonLabel_size);
        paddingCount ++;
        // protocolBluetoothGanglion = new Button (x + padding, y + padding * paddingCount + buttonHeight * paddingCount, w - padding * 2, 24, "Bluetooth", fontInfo.buttonLabel_size);
        // paddingCount ++;
        // h += padding + buttonHeight;
...
class SessionDataBox {
    int x, y, w, h, padding; //size and position
    int i; //0 for Cyton, 1 for Ganglion, 2 for Bitalino
    ...
    SessionDataBox (int _x, int _y, int _w, int _h, int _padding, int _dataSource) {
        ...
        } else if (_dataSource == DATASOURCE_GANGLION) {
            textfieldName = "fileNameGanglion";
        } else {
            textfieldName = "fileNameBitalino";
        }
        ...
        if (_dataSource == DATASOURCE_CYTON) {
            maxDurDropdownName = "maxFileDurationCyton";
        } else if (_dataSource == DATASOURCE_GANGLION) {
            maxDurDropdownName = "maxFileDurationGanglion";
        } else {
            maxDurDropdownName = "maxFileDurationBitalino";
        }
    ...
    void lockElements (boolean _toggle) {
        if (eegDataSource == DATASOURCE_CYTON) {
            ...
            */
            ...
            sampleRate100.setIgnoreHover(_toggle);
        }
    }
...
void maxFileDurationBitalino (int n) {
    settings.bitalinoOBCIMaxFileSize = n;
    controlPanel.dataLogBoxBitalino.closeDropdown();
    println("ControlPanel: Bitalino Max Recording Duration: " + settings.fileDurations[n]);
}
//////////////////////////////////////////////////////////////
...
class SampleRateBitalinoBox {
    int x, y, w, h, padding; //size and position

    SampleRateBitalinoBox(int _x, int _y, int _w, int _h, int _padding) {
        x = _x;
        y = _y;
        w = _w;
        h = 73;
        ...
        sampleRate100 = new Button (x + padding, y + padding*2 + 18, (w-padding*3)/2, 24, "100Hz", fontInfo.buttonLabel_size);
        // sampleRate1600 = new Button (x + padding*2 + (w-padding*3)/2, y + padding*2 + 18, (w-padding*3)/2, 24, "1600Hz", fontInfo.buttonLabel_size);
        // sampleRate1600.setColorNotPressed(isSelected_color); //make it appear like this one is already selected
    }
    ...
    public void draw() {
        ...
        textFont(h3, 16);
        textAlign(LEFT, TOP);
        text("  " + str((int)bitalino.getSampleRate()) + "Hz", x + padding + 142, y + padding); // print the channel count in green next to the box title
        popStyle();

        sampleRate100.draw();
        // sampleRate1600.draw();
        sampleRate100.but_y = y + padding*2 + 18;
        // sampleRate1600.but_y = sampleRate200.but_y;
    }
};
